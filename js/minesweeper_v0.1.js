...
$(document).ready(function () {
    ...
    $("body").append(createDivBoard(myrows, mycolumns));
    $("body").append($("<h1\>").attr("id", "game-result"));
});
//-------------------------------------------------------------------------------------------------
// TODO: change it or delete
function createHeader() {
    var $selectList = $("<select\>")
    $selectList.append($("<option\>").attr("selected", "").attr("value", "level_1").text("10x10 mines:15"));
    $selectList.append($("<option\>").attr("value", "level_2").text("15x15 mines:30"));
    $selectList.append($("<option\>").attr("value", "level_3").text("20x20 mines:40"));

    $selectList.on("change", changeLevel);

    var $header = $("<div\>").append($("<p\>").text("Chose level ").append($selectList));
...
//-------------------------------------------------------------------------------------------------
/**
 * Create square of divs - main game board
 *
 ...
 */
function createDivBoard(rows, columns) {
    var $mainDiv = $("<div\>").addClass("divBoard");
    var CellWidth = 15, CellBorder = 3; // That variables must to be the same as in css file
    var widthOfMainDiv = columns * (CellWidth + CellBorder * 2);
    $mainDiv.css({
        "width": widthOfMainDiv + "px",
        "height": widthOfMainDiv + "px"
    });

    var $boardElemnet;
    for (var i = 0; i < rows; i++) {
        for (var j = 0; j < columns; j++) {
            $boardElemnet = $("<div\>").attr("id", i + "-" + j).on({
                click: onClickDivBoard,
                contextmenu: showFlag
            });
            $mainDiv.append($boardElemnet);
        }
        $mainDiv.append($("<br>"));
    }
    return $mainDiv;
...
function Minesweeper(rows, columns, mines) {
    ...
    this.getData = function (rowIndex, columnIndex) {
        if (rowIndex >= 0 && rowIndex < rows && columnIndex >= 0 && columnIndex < columns) {
            return gameData[rowIndex][columnIndex].data;
        } else {
            return false;
        }
    }
    //------------------------------------------------------------------------------
    this.getFlag = function (rowIndex, columnIndex) {
        if (rowIndex >= 0 && rowIndex < rows && columnIndex >= 0 && columnIndex < columns) {
            return gameData[rowIndex][columnIndex].flag;
        } else {
            return false;
        }
    }
     ...
     */
    this.setFlag = function (rowIndex, columnIndex, value) {
        if (rowIndex >= 0 && rowIndex < rows && columnIndex >= 0 && columnIndex < columns) {
            gameData[rowIndex][columnIndex].flag = value;
            return true;
        } else {
            return false;
        }
     ...
     * @public
     */
    this.isOpened = function (rowIndex, columnIndex) {
        if (rowIndex >= 0 && rowIndex < rows && columnIndex >= 0 && columnIndex < columns) {
            return gameData[rowIndex][columnIndex].opened;
        } else {
            return false;
        }
    }
     ...
     * @public
     */
    this.open = function (rowIndex, columnIndex) {
        if (rowIndex >= 0 && rowIndex < rows && columnIndex >= 0 && columnIndex < columns) {
            gameData[rowIndex][columnIndex].opened = true;
            return true;
        } else {
            return false;
        }
    ...
    this.isGameCompleted = function () {
        for (var i = 0; i < rows; i++) {
            for (var j = 0; j < columns; j++) {
                if (gameData[i][j].data != "mine" && gameData[i][j].opened == false) {
                    return false;
                }
            }
        }
